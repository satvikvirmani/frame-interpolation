...
def select_keyframes(csv_path, abs_thresh=None, delta_thresh=None, adapt_factor=1.0, save_path=None):
    ...
    print(f"\n📊 Thresholds used:")
    print(f"  Absolute difference: {abs_thresh:.6f}")
    print(f"  Delta difference:    {delta_thresh:.6f}")

    # --- Selection logic ---
    keep_indices = [
        i for i in range(len(diffs))
        if (diffs[i] > abs_thresh) or (abs(delta_diffs[i]) > delta_thresh)
    ]
    ...
    print(f"\n✅ Keeping {len(keep_indices)}/{len(diffs)} frames "
          f"({len(keep_indices)/len(diffs)*100:.2f}%)")
    
    # --- Optionally save selected indices ---
    if save_path:
